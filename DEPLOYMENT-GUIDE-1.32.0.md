# Pool Shed v1.32.0 Deployment Guide
...
5. Run `npm run build` and deploy the generated `dist` plus the existing `api` and `server` functions through the Pool Shed Vercel project.
6. Confirm the service worker is serving the `pool-shed-v1.32.0-elite-quote-builder` cache and not an older quote-builder cache.
7. Verify `/` loads only the internal Pool Shed app and `/proposal` loads only the isolated private-client presentation.
8. Create a test Quick Quote and upload a hero image and an option image. Preview the customer proposal and confirm both appear.
9. Create an image/gallery content block and verify the media library can reuse the uploaded quote image.
10. Publish a secure test quote and confirm the public payload contains no Product Hub supplier cost, margin, stock, Sales Order, PO or internal handover fields.
11. Accept a Quick Quote and verify one Sales Order is created; verify no Project is created unless `Create Project on acceptance` was selected.
12. Test a Project Proposal and verify Project + Sales Order + stock/procurement handover remains linked to the frozen version.
13. Confirm Azzy readability, responsive page fit, Product Hub, Sales Orders, Purchasing, Warehouse, Projects and Settings remain operational.
14. Only after the smoke tests pass should v1.32 become the production baseline.
