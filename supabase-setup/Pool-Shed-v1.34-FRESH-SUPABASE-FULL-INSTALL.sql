...
-- ============================================================
-- BEGIN 011-project-extra-quotes.sql
-- ============================================================
-- Run after 010-product-crm-media.sql. Extends project history protection for linked extra/credit quote acceptance.
begin;
create or replace function public.ps_project_history_guard() returns trigger language plpgsql security definer set search_path=public as $$
declare old_job jsonb; new_job jsonb; old_item jsonb; new_item jsonb;
begin
 for old_job in select value from jsonb_array_elements(coalesce(old.data->'jobs','[]'::jsonb)) where value ? 'project' loop
  ...
  for old_item in select value from jsonb_array_elements(coalesce(old_job->'project'->'quoteLinks','[]'::jsonb)) where value->>'status'='Accepted' loop
   if not exists(select 1 from jsonb_array_elements(coalesce(new_job->'project'->'quoteLinks','[]'::jsonb)) ql where ql=old_item) then raise exception 'Accepted project quote linkage and approval evidence must be retained'; end if;
  end loop;
  for new_item in select value from jsonb_array_elements(coalesce(new_job->'project'->'variations','[]'::jsonb)) where value->>'status'='Approved' and coalesce(value->>'quoteId','')<>'' loop
   if not exists(select 1 from jsonb_array_elements(coalesce(old_job->'project'->'variations','[]'::jsonb)) ov where ov->>'id'=new_item->>'id' and ov->>'status'='Approved') then
    if coalesce(new_item->>'approvalRef','')='' or not exists(select 1 from jsonb_array_elements(coalesce(new.data->'quotes','[]'::jsonb)) q where q->>'id'=new_item->>'quoteId' and q->>'status'='Accepted' and q->'acceptance' is not null) then raise exception 'Quote-linked extra approval requires accepted quote evidence'; end if;
   end if;
  end loop;
  for new_item in select value from jsonb_array_elements(coalesce(new_job->'project'->'quoteLinks','[]'::jsonb)) where value->>'status'='Accepted' and value->>'role' in ('Extra','Credit') loop
   if not exists(select 1 from jsonb_array_elements(coalesce(old_job->'project'->'quoteLinks','[]'::jsonb)) oql where oql->>'quoteId'=new_item->>'quoteId' and oql->>'status'='Accepted') then
    if coalesce(new_item->>'approvalRef','')='' or not exists(select 1 from jsonb_array_elements(coalesce(new.data->'quotes','[]'::jsonb)) q where q->>'id'=new_item->>'quoteId' and q->>'status'='Accepted' and q->'acceptance' is not null) then raise exception 'Accepted extra quote link requires customer acceptance evidence'; end if;
   end if;
  end loop;
  for old_item in select value from jsonb_array_elements(coalesce(old_job->'project'->'costs','[]'::jsonb)) loop
   select c into new_item from jsonb_array_elements(coalesce(new_job->'project'->'costs','[]'::jsonb)) c where c->>'id'=old_item->>'id';
...
-- ============================================================
-- END 011-project-extra-quotes.sql
-- ============================================================
