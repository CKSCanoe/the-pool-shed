...
-- Recreate reporting views so rerunning this schema is safe even when an older
-- version of a view used different numeric precision/scale.
drop view if exists public.location_stock_values cascade;

create view public.location_stock_values as
select
  l.id as location_id,
...
create view public.location_restock_requirements as
select
...
left join public.stock_balances sb on sb.product_id = lrr.product_id and sb.location_id = lrr.location_id
...
create policy "admins insert profiles" on public.user_profiles for insert to authenticated with check (public.current_user_is_admin());
drop policy if exists "admins update profiles" on public.user_profiles;
create policy "admins update profiles" on public.user_profiles for update to authenticated using (public.current_user_is_admin()) with check (public.current_user_is_admin());
