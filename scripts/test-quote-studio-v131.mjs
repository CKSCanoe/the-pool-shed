...
assert.match(pkg.version,/^1\.32\.[1-9]\d*$/);
...
assert.match(index,/quote-studio-engine\.js\?v=1\.32\.1/);
assert.match(index,/quote-studio-workspace\.js\?v=1\.32\.1/);
...
assert.match(sw,/pool-shed-v1\.32\.1-elite-quote-builder/);
...
const qs=context.PoolShedQuoteStudio;assert(qs,'Quote Studio engine must initialise');assert.equal(qs.VERSION,'1.32.1');
...
console.log('PASS v1.32.1 Quote Studio native wiring, one-state engine, safe public snapshot and SKU/bundle operational mapping');
