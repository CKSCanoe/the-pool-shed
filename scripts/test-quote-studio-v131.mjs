...
assert.match(index,/quote-studio-engine\.js\?v=1\.41\.0/);
assert.match(index,/quote-studio-workspace\.js\?v=1\.41\.0/);
...
assert.match(sw,/pool-shed-v1\.41\.0-quote-studio-command/);
...
const qs=context.PoolShedQuoteStudio;assert(qs,'Quote Studio engine must initialise');assert.equal(qs.VERSION,'1.41.0');
...
console.log('PASS v1.41.0 Quote Studio native wiring, one-state engine, safe public snapshot and SKU/bundle operational mapping');
