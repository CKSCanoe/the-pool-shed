...
assert.match(index,/quote-studio\.css\?v=1\.29\.0/);
assert.match(index,/quote-studio-engine\.js\?v=1\.29\.0/);
assert.match(index,/quote-studio-workspace\.js\?v=1\.29\.0/);
assert.match(legacy,/quotes:\s*\[\]/);
assert.match(legacy,/id:\s*"quotes",\s*label:\s*"Quote Studio"/);
...
assert.match(sw,/pool-shed-v1\.29\.0-quote-studio-operational-handover/);
...
const qs=context.PoolShedQuoteStudio;assert(qs,'Quote Studio engine must initialise');assert.equal(qs.VERSION,'1.29.0');
const q=qs.createQuote({customerId:'C1',projectName:'Test Pool',projectType:'Pool Build'}),section=q.sections[0];
...
console.log('PASS v1.29.0 Quote Studio native wiring, one-state engine, safe public snapshot and SKU/bundle operational mapping');
