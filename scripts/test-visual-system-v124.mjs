...
if (pkg.version !== '1.24.0') failures.push(`package version must be 1.24.0, found ${pkg.version}`);
...
if (!/Pool Shed v1\.24\.0 · Pool Bros Ltd/.test(legacy)) failures.push('discreet footer/version must report v1.24.0');
...
if (!html.includes('app.css?v=1.24.0')) failures.push('app.css runtime version must be v1.24.0');
if (!html.includes('config.js?v=1.24.0')) failures.push('runtime JS version must be v1.24.0');
if (!/1\.24\.0/.test(sw)) failures.push('service worker cache/runtime must reference v1.24.0');
