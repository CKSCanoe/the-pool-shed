...
vm.runInContext(fs.readFileSync('public/project-workspace.js','utf8'),c);c.render=()=>{screen.innerHTML=c.psProjectWorkspace()};c.openSidebarSubGroup=c.render;vm.runInContext(fs.readFileSync('public/project-design-parity.js','utf8'),c);
...
c.openSidebarSubGroup('jobs','Settings');assert(screen.innerHTML.includes('name="settingsReason"'));
c.openSidebarSubGroup('jobs','Costing & Margin');assert(screen.innerHTML.includes('data-pl-form="time"'));assert(!screen.innerHTML.includes('data-project-tab="Settings"'));
...
c.openSidebarSubGroup('jobs','All projects');assert(screen.innerHTML.includes('data-project-search-form'));
