...
assert.throws(()=>c.psToolTransaction('allocate',{id:tool.id}),/not available/);
assert.throws(()=>c.psToolTransaction('save',{id:tool.id,name:'Changed',replacementValue:1,dailyRate:99}),/Return/);
c.psToolTransaction('return',{id:tool.id,condition:'Good',kit0:'on'});assert.equal(c.data.toolAssets[0].status,'Inspection required');assert.equal(c.data.toolAssignments[0].missingItems[0],'Charger');assert.equal(c.psToolJobOutstanding('JOB').length,0);
c.psToolTransaction('service',{id:tool.id});assert.equal(c.data.toolAssets[0].status,'Available');assert.equal(c.psToolCost({startedAt:'2026-01-01T00:00:00Z',returnedAt:'2026-01-03T00:00:00Z',dailyRate:20,ownership:'Owned'}),40);
tool=c.psToolTransaction('save',{name:'Breaker',ownership:'Hired In',replacementValue:500,dailyRate:35});c.psToolTransaction('allocate',{id:tool.id,jobId:'JOB',responsible:'Engineer',expectedReturn:'2030-01-01'});c.psToolTransaction('return',{id:tool.id,condition:'Good'});assert.equal(c.psToolJobOutstanding('JOB').length,1);c.psToolTransaction('offhire',{id:tool.id});assert.equal(c.psToolJobOutstanding('JOB').length,0);
assert.equal(c.psToolCost({startedAt:'2026-01-01T00:00:00Z',returnedAt:'2026-01-02T00:00:00Z',offHireAt:'2026-01-04T00:00:00Z',dailyRate:35,ownership:'Hired In'}),105);
