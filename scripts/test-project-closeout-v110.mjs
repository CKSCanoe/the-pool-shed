...
for(const code of ['JOB_BIN_STOCK','OPEN_PO','OUTSTANDING_TOOL','PROPOSED_VARIATION','BILLING_INCOMPLETE'])assert(blockers.some(b=>b.code===code),`Expected ${code} blocker`);
// Clear operational blockers and make billing reconciled.
data.stock[0].qty=0;data.purchaseOrders[0].lines[0].received=3;data.toolAssignments[0].returnedAt='2026-09-15';p.variations[0].status='Rejected';p.phases[0].invoiceRequested=true;
...
blockers=c.psProjectCloseoutBlockers(job,data,{documents:[{source_id:'PROJECT:P1:PH1',status:'AUTHORISED'}]},Date.parse('2026-09-15T12:00:00Z'));
assert(!blockers.some(b=>b.code==='COMMERCIAL_REVIEW'));
console.log('Project close-out: stock, PO, tool, variation, billing and critical commercial review gates passed.');
