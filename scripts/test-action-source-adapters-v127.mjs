...
assert.deepEqual(new Set(registered),new Set(['purchasing','sales','warehouse','projects','finance','automation','engineer']));
const d={
 purchaseOrders:[
 ...
 engineerRequests:[{id:'ER-1',status:'Open',engineer:'Dave',projectId:'J-1',title:'Need pump',requestedAt:'2026-09-17T09:00:00Z'}]
};
...
const er=adapters.engineer(d);assert.ok(er.some(x=>x.dedupeKey==='engineer-request:ER-1:follow-up'));
for(const rows of [p,s,w,pj,f,au,er])for(const x of rows){assert.ok(x.ownerRole);assert.ok(x.source&&x.source.module&&x.source.route);assert.ok(x.dedupeKey);}
console.log('PASS v1.27 source adapters for Purchasing, Sales, Warehouse, Projects, Finance, Automation and Engineer Requests');
