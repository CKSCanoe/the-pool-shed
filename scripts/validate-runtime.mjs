...
for (const file of [htmlPath, swPath, configPath, salesSearchPath, salesSearchCssPath, partialFulfilmentPath, partialFulfilmentCssPath]) {
...
for (const asset of ['sales-order-search.css', 'sales-order-search.js', 'partial-fulfilment.css', 'partial-fulfilment.js']) {
  if (!swSource.includes(asset)) throw new Error(`Offline cache is missing ${asset}`);
}
if (!swSource.includes('pool-shed-v1.10.9-partial-shipments')) {
  throw new Error('Service worker cache version was not advanced to v1.10.9');
}
...
console.log(`Runtime validation passed: ${count} inline scripts, service worker, offline search, partial shipment balance test, premium UI, required app functions and 6 smart-search tests.`);
