...
for (const file of localAssetPaths.filter(file => file.endsWith('.js'))) {
  try { new vm.Script(fs.readFileSync(file, 'utf8'), { filename: path.relative(root, file) }); }
  catch (error) { throw new Error(`External JavaScript failed syntax validation (${path.relative(root, file)}): ${error.message}`); }
}
if (!count && !localAssetPaths.some(file => file.endsWith('.js'))) throw new Error('No application scripts were found');
...
for (const requiredStatusControl of ['status-picker-sales', 'status-picker-po', 'status-picker-label', 'status-picker-native', 'salesOrderStatusPicker', 'purchaseOrderStatusPicker', 'aria-label=\"Sales order status\"', 'aria-label=\"Purchase order status\"']) {
  if (!appSource.includes(requiredStatusControl)) throw new Error(`Status picker is incomplete: ${requiredStatusControl}`);
}
if (appSource.includes('<div class=\"so-status-control\"')) {
  throw new Error('Legacy blank-prone sales order status control remains');
}
if ((appSource.match(/purchaseOrderStatusPicker\(po/g) || []).length < 5) {
...
const releaseVersion = html.match(/assets\/css\/app\.css\?v=([0-9.]+)/)?.[1];
if (!releaseVersion || !swSource.includes(`pool-shed-v${releaseVersion}-`)) {
  throw new Error(`Service worker cache key is not set for the current release ${releaseVersion || 'unknown'}`);
}
...
for (const requiredTitleFeature of ['pb-product-title-persistence-v115-fix', 'data-product-parent-title', 'Product catalogue title changed']) {
  if (!appSource.includes(requiredTitleFeature)) throw new Error(`Product title persistence is missing ${requiredTitleFeature}`);
}

const productImagesSource = fs.readFileSync(productImagesPath, 'utf8');
new vm.Script(productImagesSource, { filename: 'product-images.js' });
...
for (const requiredImageFeature of [
  ...
  'parentImageUrl',
  'variantImageUrl',
  'data-parent-image-url',
  'data-product-image-shell',
  'pb-product-image-fallback'
]) {
  if (!productImagesSource.includes(requiredImageFeature)) throw new Error(`Product image feature missing: ${requiredImageFeature}`);
}
for (const requiredImageStyle of [
  '.pb-product-image',
  '.pb-product-image-xl',
  '.variant-image-cell',
  '.product-profile-image-preview-wrap',
  '.ci-result-thumb',
  '.so-result-thumb'
]) {
  if (!productImagesCss.includes(requiredImageStyle)) throw new Error(`Product image styling is incomplete: ${requiredImageStyle}`);
}
for (const requiredImageIntegration of [
  'Shared parent product image',
  'Variant image override',
  'parentImageUrl',
  'PoolShedProductImages.markup(first',
  'PoolShedProductImages.markup(p'
]) {
  if (!appSource.includes(requiredImageIntegration)) throw new Error(`Product image integration missing: ${requiredImageIntegration}`);
}
...
if (html.indexOf('./product-images.js') > html.indexOf('./sales-order-search.js')) {
  throw new Error('product-images.js must load before catalogue and order search assets');
}
...
if (appSource.includes('<div class="inventory-sync-banner">')) {
...
if (!imageDiagnostics || imageDiagnostics.version !== '1.44.1') {
  throw new Error('Product image diagnostics API did not initialise');
}
context.data.products = [
  { id: 'P-IMG-1', parentSku: 'PB-GRP-IMG', parentName: 'Image group', sku: 'PB-IMG-1', parentImageUrl: 'https://example.com/parent.jpg', imageUrl: '' },
  { id: 'P-IMG-2', parentSku: 'PB-GRP-IMG', parentName: 'Image group', sku: 'PB-IMG-2', imageUrl: 'https://example.com/variant.jpg' }
];
if (imageDiagnostics.variantImageUrl(context.data.products[0]) !== 'https://example.com/parent.jpg') {
  throw new Error('Variant did not inherit the shared parent image');
}
if (imageDiagnostics.variantImageUrl(context.data.products[1]) !== 'https://example.com/variant.jpg') {
  throw new Error('Variant image override did not take priority');
}
if (!imageDiagnostics.markup({ id: 'P-NO-IMAGE', sku: 'PB-NO-IMAGE' }, { mode: 'variant' }).includes('pb-product-image-fallback')) {
  throw new Error('Missing product images do not retain the PB fallback');
}
...
if (!catalogueDiagnostics || catalogueDiagnostics.version !== '1.11.8') {
...
if (appSource.includes("reconcileInventory('Product Hub render')")) {
  throw new Error('Unsafe inventory reconciliation remains inside Product Hub render');
}
if (!appSource.includes('scheduleHubRender') || !appSource.includes('data-delete-po')) {
  throw new Error('Product Hub safe search or PO deletion controls are missing');
}
if (appSource.includes("hubState[f.dataset.hubFilter]=f.value;render()")) {
...
for (const requiredFix of ['stockRowsForLocationScope', 'dashboardHealthSummary', 'source.statuses = seed.statuses.map(clone)', 'L-WH-A1', 'orderedDashboardStatuses']) {
  if (!appSource.includes(requiredFix)) throw new Error(`Dashboard/inventory fix missing: ${requiredFix}`);
}
for (const required of ['matchesSearchProduct', 'renderProducts', 'render', 'saveAppData', 'deletePurchaseOrder']) {
  if (!appSource.includes(required)) throw new Error(`Required application function missing: ${required}`);
}
...
// 12 physically in stock and 12 still allocated for later goods notes.
{
  ...
  const partialContext = {
    ...
    allocationRowsForSalesOrder(id) { return testStock.filter(row => row.productId === id); },
    isLocationStockTakeFrozen() { return false; },
...
for (const requiredHealthAction of [
  ...
  'catalogueHealthImportFile',
  'No new products will be created',
  'type="button" class="secondary" data-health-export',
  'type="button" data-health-import'
]) {
  if (!appSource.includes(requiredHealthAction)) throw new Error(`Catalogue Health action missing: ${requiredHealthAction}`);
}

for (const marker of ['ci-mega-body', 'ci-po-basket', 'data-ci-basket-confirm', 'commitPoBasket']) { if (!catalogueSource.includes(marker)) throw new Error('Missing v1.11.8 purchasing mega search marker: ' + marker); }
