...
if (!html.includes('./product-images.css') || !html.includes('./product-images.js')) {
...
for (const feature of ['Catalogue quality workspace','data-health-save','data-health-export','data-health-import','bindCatalogueHealth']) {
  if (!html.includes(feature)) throw new Error(`Catalogue health control is missing ${feature}`);
}
...
for (const requiredTitleFeature of ['pb-product-title-persistence-v115-fix', 'data-product-parent-title', 'Product catalogue title changed']) {
  if (!html.includes(requiredTitleFeature)) throw new Error(`Product title persistence is missing ${requiredTitleFeature}`);
}
...
for (const requiredImageIntegration of [
  ...
  if (!html.includes(requiredImageIntegration)) throw new Error(`Product image integration missing: ${requiredImageIntegration}`);
}
...
for (const requiredHealthAction of [
  ...
  if (!html.includes(requiredHealthAction)) throw new Error(`Catalogue Health action missing: ${requiredHealthAction}`);
}
