...
const js=fs.readFileSync(new URL('../public/bundle-studio-v224.js',import.meta.url),'utf8');
const css=fs.readFileSync(new URL('../public/bundle-studio-v224.css',import.meta.url),'utf8');
...
const checks=[
  ...
  ['assets included',html.includes('bundle-studio-v224.css')&&html.includes('bundle-studio-v224.js')]
];
const failed=checks.filter(([,ok])=>!ok);
if(failed.length){console.error(failed.map(([name])=>'FAIL: '+name).join('\n'));process.exit(1)}
console.log('Bundle Studio v2.2.4 checks: Passed');
