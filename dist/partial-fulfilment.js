...
(function () {
  ...
  function releaseShipmentAllocation(order, line, qty) {
    ...
    let left = releaseQty;
    allocationRowsForSalesOrder(line.productId, order.id).slice().sort(function (a, b) {
  ...
  function shipmentDeductionPlan(order, note) {
    ...
    for (const noteLine of note.lines || []) {
      if (!Number.isFinite(Number(noteLine.qty)) || num(noteLine.qty) <= 0 || seen.has(noteLine.productId)) return null;
      seen.add(noteLine.productId);
      ...
      const rows = allocationRowsForSalesOrder(noteLine.productId, order.id).filter(function(row) {return !["L-QUARANTINE", "L-RECEIVING"].includes(row.locationId) && !isLocationStockTakeFrozen(row.locationId);}).slice().sort(function (a, b) {
  ...
  allocateSalesOrderLine = function (orderId, productId, quiet) {
    ...
    let left = needed;
    allocationRowsForSalesOrder(productId, order.id).slice().sort(function (a, b) { return available(b) - available(a); }).forEach(function (row) {
      if (!left) return;
  ...
  shipGoodsNote = function (note) {
    ...
    syncSalesOrderStatusFromGoodsNotes(order);
    addNotification(note, 'Ship', 'Your order has shipped with ' + (note.courier || 'Courier') + ' tracking ' + (note.trackingRef || 'Tracking pending'), 'Ready to send');
    saveAppData();
    return true;
  };
