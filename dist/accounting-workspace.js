...
(function(){
 ...
 function paint(){
  ...
  const c=state?.connection,xi=state?.integration||{},live=!!xi.liveEnabled,ready=!!xi.readyToConnect;
  target.innerHTML='<header><h2>Accounting & Xero</h2><p class="muted">Review drafts here. Approve them in Xero. Payment balances return automatically once the scheduled sync is configured.</p></header>'+
   ...
   '<div class="action-row"><button data-finance="refresh">Refresh</button>'+(live?'<button data-finance="connect">'+(c?'Reconnect Xero':'Connect Xero')+'</button>':'<button type="button" disabled>Live connection locked</button>')+(c&&live?'<button data-finance="tenants">Select organisation</button><button data-finance="sync">Check for updates</button>':'')+'</div>'+
   '<p><strong>'+esc(c?.tenant_name||(ready?'Ready to Connect':'Not connected'))+'</strong> · '+(busy?'Working…':c?.last_sync?'Last checked '+esc(new Date(c.last_sync).toLocaleString()):ready?'Live connection locked until production acceptance':'No successful sync yet')+'</p>'+(c?.last_error?'<p role="alert">'+esc(c.last_error)+'</p>':'')+
