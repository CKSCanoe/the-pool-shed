(function(){
  ...
  function scheduleHubRender(delay){
    ...
    const field=focusedElement&&focusedElement.matches&&focusedElement.matches('[data-hub-filter]')?focusedElement.dataset.hubFilter:'';
    const start=focusedElement&&typeof focusedElement.selectionStart==='number'?focusedElement.selectionStart:null;
    const end=focusedElement&&typeof focusedElement.selectionEnd==='number'?focusedElement.selectionEnd:null;
    hubRenderTimer=setTimeout(function(){
      if(active!=='products'||productView!=='list')return;
      renderProducts();
      if(field){
  ...
  setTimeout(function(){const r=reconcileInventory('v1.10.3 startup reconciliation');if(r.duplicatesMerged)saveAppData();if(active==='products')render()},150);
})();
