...
(function () {
  ...
  function selectedSummaryHtml(productRecord, order) {
    ...
    const image = window.PoolShedProductImages ? window.PoolShedProductImages.markup(productRecord, { mode: 'variant', className: 'so-selected-thumb', size: 'md', alt: parent || productRecord.name }) : '';
    return image + '<div class="identity"><strong>' + escape(parent || productRecord.name || 'Selected product') + '</strong>' +
  ...
  function resultHtml(productRecord, order, index) {
    ...
    const image = window.PoolShedProductImages ? window.PoolShedProductImages.markup(productRecord, { mode: 'variant', className: 'so-result-thumb', size: 'md', alt: displayName }) : '<span class="pb-product-image pb-product-image-md is-fallback so-result-thumb"><span class="pb-product-image-fallback">PB</span></span>';
    return '<button type="button" role="option" aria-selected="false" tabindex="-1" class="so-product-result ' + stock.statusClass + '" data-so-select-product-v1105="' + escape(order.id) + '|' + escape(productRecord.id) + '" data-search-index="' + index + '">' + image +
