...
(function () {
  ...
  function buildCatalogueCache() {
    ...
    products.forEach(function (entry) { if (entry.product && entry.product.id != null) entryById.set(str(entry.product.id), entry); });
    catalogueCache = { products: products, parentById: parentById, parentBySku: parentBySku, entryById: entryById };
  ...
  function rankProducts(query, limit) {
    const entries = buildCatalogueCache().products;
    const max = limit || 24;
    if (!normalise(query)) {
  ...
  function positionPopover(box, input) {
    ...
    box.style.width = Math.min(Math.max(shellRect.width, input.id === 'poProductSearch' ? 820 : 680), hostRect.width - 24) + 'px';
    box.style.height = 'auto';
    box.style.maxHeight = input.id === 'poProductSearch' ? Math.min(640, availableBelow) + 'px' : Math.min(520, availableBelow) + 'px';
