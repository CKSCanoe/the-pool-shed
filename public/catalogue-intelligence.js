...
(function () {
  ...
  function ensureSearchBackdrop() {
    if (searchBackdrop && searchBackdrop.isConnected) return searchBackdrop;
    searchBackdrop = document.createElement('button');
    searchBackdrop.type = 'button';
    searchBackdrop.className = 'ci-search-backdrop';
    searchBackdrop.hidden = true;
    searchBackdrop.setAttribute('aria-label', 'Close catalogue search results');
  ...
  function setActiveSearchSource(input) {
    document.querySelectorAll('.ci-search-source-active').forEach(function (node) {
      node.classList.remove('ci-search-source-active');
    });
    if (!input || !input.isConnected) return;
    const shell = input.closest('.ci-search-shell');
    if (shell) shell.classList.add('ci-search-source-active');
  }
  function showSearchLayer(input) {
    const backdrop = ensureSearchBackdrop();
    backdrop.hidden = false;
    setActiveSearchSource(input);
    document.body.classList.add('ci-search-layer-open');
  }
  function tidySearchLayer() {
    const hubOpen = Boolean(hubPopover && hubPopover.isConnected && !hubPopover.hidden);
    const poOpen = Boolean(poPopover && poPopover.isConnected && !poPopover.hidden);
    if (hubOpen || poOpen) return;
    if (searchBackdrop) {
      searchBackdrop.hidden = true;
  ...
  function positionPopover(box, input) {
    ...
    const margin = window.innerWidth <= 760 ? 10 : 24;
    const maxWidth = Math.min(980, window.innerWidth - margin * 2);
    const minWidth = Math.min(680, maxWidth);
    const width = Math.min(Math.max(rect.width + 170, minWidth), maxWidth);
    const left = Math.max(margin, Math.min(rect.left + (rect.width - width) / 2, window.innerWidth - width - margin));
    const desiredHeight = Math.min(560, window.innerHeight - margin * 2);
    const roomBelow = window.innerHeight - rect.bottom - margin;
    const roomAbove = rect.top - margin;
    const placeAbove = roomBelow < 390 && roomAbove > roomBelow;
    const availableHeight = Math.max(300, Math.min(desiredHeight, placeAbove ? roomAbove - 12 : roomBelow - 12));
    const top = placeAbove
      ? Math.max(margin, rect.top - availableHeight - 12)
      : Math.min(rect.bottom + 12, window.innerHeight - availableHeight - margin);
