...
(function () {
  ...
  let supplierRowsCache = null;
  let supplierProfileCache = new Map();
  let catalogueCacheSignature = '';
  let catalogueCacheBuiltAt = 0;
  ...
  function fastCompactIdentifier(value) { return str(value).toLowerCase().replace(/[^a-z0-9]+/g, ''); }
  function rawExactProduct(query) {
    const key = fastCompactIdentifier(query);
    if (!key) return null;
    const source = (typeof data !== 'undefined' && Array.isArray(data.products)) ? data.products : [];
    for (let i = 0; i < source.length; i += 1) {
      const p = source[i];
      if (!p || p.deleted || p.hiddenFromCatalogue || p.active === false || p.archived === true) continue;
      const ids = [p.sku,p.code,p.itemCode,p.item_code,p.barcode,p.upc,p.ean,p.supplierSku,p.supplier_sku,p.parentSku,p.parent_sku];
      for (let j = 0; j < ids.length; j += 1) if (ids[j] && fastCompactIdentifier(ids[j]) === key) return p;
    }
    return null;
  }
  function catalogueSignature() {
    const source = (typeof data !== 'undefined' && Array.isArray(data.products)) ? data.products : [];
    const parents = (typeof data !== 'undefined' && (data.productParents || data.product_parents)) || [];
    const supplierRows = (typeof data !== 'undefined' && Array.isArray(data.supplierProducts)) ? data.supplierProducts : [];
    const first = source[0] || {};
    const last = source[source.length - 1] || {};
    return [source.length, parents.length, supplierRows.length, first.id || first.sku || '', last.id || last.sku || ''].join('|');
  }
  function buildCatalogueCache() {
    const signature = catalogueSignature();
    if (catalogueCache && catalogueCacheSignature === signature && (Date.now() - catalogueCacheBuiltAt) < 30000) return catalogueCache;
    const source = (typeof data !== 'undefined' && Array.isArray(data.products)) ? data.products : [];
    const parents = (typeof data !== 'undefined' && (data.productParents || data.product_parents)) || [];
    const parentById = new Map();
    const parentBySku = new Map();
    parents.forEach(function (parent) {
      if (parent && parent.id) parentById.set(str(parent.id), parent);
      const sku = compact(parent && (parent.parentSku || parent.parent_sku));
      if (sku) parentBySku.set(sku, parent);
    });
    const ids = new Set();
    const skus = new Set();
    const products = [];
    source.forEach(function (productRecord) {
      if (!productRecord || productRecord.deleted || productRecord.hiddenFromCatalogue) return;
      if (productRecord.active === false || productRecord.archived === true || normalise(productRecord.status) === 'archived') return;
      const id = str(productRecord.id).trim();
      const sku = compact(productRecord.sku || productRecord.code || productRecord.itemCode || '');
      if ((id && ids.has(id)) || (sku && skus.has(sku))) return;
      if (id) ids.add(id);
      if (sku) skus.add(sku);
      if (!(id || sku || productRecord.name)) return;
      const parentId = str(productRecord.parentProductId || productRecord.parent_product_id || '');
      const parentSku = compact(productRecord.parentSku || productRecord.parent_sku || '');
      const parent = (parentId && parentById.get(parentId)) || (parentSku && parentBySku.get(parentSku)) || null;
      const parentTitle = productRecord.parentName || productRecord.parent_name || (parent && parent.name) || '';
      const variant = productRecord.variantValue || productRecord.variant_value || productRecord.variantLabel || productRecord.variant_label || productRecord.variant || productRecord.packSize || productRecord.pack_size || '';
      const identifiers = [productRecord.sku, productRecord.code, productRecord.itemCode, productRecord.item_code, productRecord.barcode, productRecord.upc, productRecord.ean, productRecord.supplierSku, productRecord.supplier_sku, productRecord.parentSku, productRecord.parent_sku].filter(Boolean).map(compact);
      const searchText = normalise([parentTitle, productRecord.parentSku, productRecord.parent_sku, parent && (parent.parentSku || parent.parent_sku), productRecord.name, variant, productRecord.sku, productRecord.code, productRecord.itemCode, productRecord.supplierSku, productRecord.supplier_sku, productRecord.barcode, productRecord.upc, productRecord.ean, productRecord.brand, productRecord.category, productRecord.reportingCategory, productRecord.reportingSubcategory, productRecord.supplier, productRecord.supplierName, productRecord.description, productRecord.mainDescription, productRecord.packSize, productRecord.pack_size, productRecord.unit].filter(Boolean).join(' '));
      products.push({ product: productRecord, parent: parent, parentName: parentTitle, variantName: variant, identifiers: identifiers, searchText: searchText, compactText: searchText.replace(/\s+/g, ''), words: searchText.split(' ') });
    });
    const supplierMap = new Map();
    const rows = (typeof data !== 'undefined' && Array.isArray(data.supplierProducts)) ? data.supplierProducts : [];
    rows.forEach(function (row) {
      if (!row || row.available === false || !row.productId) return;
      const key = str(row.productId);
      if (!supplierMap.has(key)) supplierMap.set(key, []);
      supplierMap.get(key).push(row);
    });
    catalogueCacheSignature = signature;
    catalogueCacheBuiltAt = Date.now();
    const entryById = new Map();
    const tokenMap = new Map();
    products.forEach(function (entry, index) {
      if (entry.product && entry.product.id != null) entryById.set(str(entry.product.id), entry);
      const uniqueWords = new Set(entry.words || []);
      uniqueWords.forEach(function (word) {
        if (!word) return;
        if (!tokenMap.has(word)) tokenMap.set(word, []);
        tokenMap.get(word).push(index);
      });
    });
    catalogueCache = { products: products, parentById: parentById, parentBySku: parentBySku, entryById: entryById, tokenMap: tokenMap };
    supplierRowsCache = supplierMap;
    supplierProfileCache = new Map();
    return catalogueCache;
  }
  function invalidateCatalogueCache() {
    catalogueCache = null;
    supplierRowsCache = null;
    catalogueCacheSignature = '';
    catalogueCacheBuiltAt = 0;
    supplierProfileCache = new Map();
  }
  function activeProducts() {
    return buildCatalogueCache().products.map(function (entry) { return entry.product; });
  }
  function tokenScore(entry, query) {
    const q = normalise(query);
    const cq = q.replace(/\s+/g, '');
    if (!q) return 1;
    if (entry.identifiers.includes(cq)) return 10000;
    if (entry.identifiers.some(function (id) { return id.indexOf(cq) === 0; })) return 7000;
    if (entry.compactText.indexOf(cq) !== -1) return 3500;
    if (entry.searchText.indexOf(q) !== -1) return 3000;
    const tokens = q.split(' ').filter(Boolean);
    let matched = 0;
    let prefix = 0;
    for (let i = 0; i < tokens.length; i += 1) {
      const token = tokens[i];
      if (entry.searchText.indexOf(token) !== -1) matched += 1;
      else if (token.length >= 3) {
        if (entry.words.some(function (word) { return word.indexOf(token) === 0 || token.indexOf(word) === 0; })) prefix += 1;
      }
    }
    if (!matched && !prefix) return -1;
    return (matched * 450) + (prefix * 180) - Math.max(0, tokens.length - matched - prefix) * 120;
  }
  function rankProducts(query, limit) {
    const max = limit || 24;
    const cleanQuery = normalise(query);
    if (cleanQuery) {
      const exactProduct = rawExactProduct(query);
      if (exactProduct) return [{ product: exactProduct, score: 10000, exact: true }];
    }
    const cache = buildCatalogueCache();
    const entries = cache.products;
    if (!cleanQuery) {
      return entries.slice(0, max).map(function (entry) { return { product: entry.product, score: 1, exact: false }; });
    }
    const cq = compact(query);
    const queryTokens = cleanQuery.split(' ').filter(Boolean);
    let candidateIndexes = null;
    if (cache.tokenMap && queryTokens.length) {
      const exactLists = queryTokens.map(function (token) { return cache.tokenMap.get(token) || null; }).filter(Boolean).sort(function (a,b) { return a.length-b.length; });
      if (exactLists.length) {
        candidateIndexes = exactLists[0];
        if (exactLists.length > 1 && candidateIndexes.length > 1) {
          const allowed = exactLists.slice(1).map(function (list) { return new Set(list); });
          candidateIndexes = candidateIndexes.filter(function (idx) { return allowed.every(function (set) { return set.has(idx); }); });
        }
      }
    }
    const scored = [];
    const sourceIndexes = candidateIndexes && candidateIndexes.length ? candidateIndexes : null;
    const count = sourceIndexes ? sourceIndexes.length : entries.length;
    for (let i = 0; i < count; i += 1) {
      const entry = sourceIndexes ? entries[sourceIndexes[i]] : entries[i];
      const score = tokenScore(entry, query);
      if (score > 0) scored.push({ product: entry.product, score: score, exact: entry.identifiers.includes(cq) });
    }
    scored.sort(function (a, b) { return b.score - a.score || str(a.product.name).localeCompare(str(b.product.name)); });
  ...
  function cachedEntry(productRecord) {
    const cache = buildCatalogueCache();
    return cache.entryById.get(str(productRecord && productRecord.id)) || null;
  }
  function parentRecord(productRecord) {
    const entry = cachedEntry(productRecord);
    return entry ? entry.parent : null;
  }
  function parentName(productRecord) {
    const entry = cachedEntry(productRecord);
    return entry ? entry.parentName : (productRecord.parentName || productRecord.parent_name || '');
  }
  function variantName(productRecord) {
    const entry = cachedEntry(productRecord);
    return entry ? entry.variantName : (productRecord.variantValue || productRecord.variant_value || productRecord.variantLabel || productRecord.variant_label || productRecord.variant || productRecord.packSize || productRecord.pack_size || '');
  }
  ...
  function hubCommandHtml(resultText) {
    ...
    return '<section class="ci-command-centre" aria-label="Catalogue intelligence search">' +
      '<div class="ci-command-head"><div><span class="ci-eyebrow">Pool Bros catalogue intelligence</span><h3>Find the exact product in seconds</h3><p>Search names, sizes, variants, Pool Bros SKUs, supplier codes and barcodes with typo tolerance, intelligent ranking and offline catalogue support.</p></div>' + connectedBadge() + '</div>' +
  ...
  function enhanceProductHub() {
    ...
    if (eyebrow) eyebrow.textContent = 'CATALOGUE INTELLIGENCE';
  }
  ...
  function positionPopover(box, input) {
    ...
    if (input.id === 'poProductSearch') {
      ...
      document.body.classList.add('ci-po-modal-open');
      return;
    }
    const host = input.closest('.ci-command-centre') || input.parentElement;
    ...
    const availableBelow = Math.max(320, window.innerHeight - shellRect.bottom - viewportGap);
    box.style.width = Math.min(Math.max(shellRect.width, 680), hostRect.width - 24) + 'px';
    box.style.height = 'auto';
    box.style.maxHeight = Math.min(520, availableBelow) + 'px';
    box.style.visibility = 'visible';
  }
  ...
  function supplierProfileSafe(name) {
    const key = str(name || 'Unknown supplier');
    if (supplierProfileCache.has(key)) return supplierProfileCache.get(key);
    const profile = typeof supplierProfile === 'function' ? (supplierProfile(name) || { name: key }) : { name: key, preferred: false, leadTimeDays: 0, creditLimit: 0 };
    supplierProfileCache.set(key, profile);
    return profile;
  }
  function supplierRowsForProduct(productRecord) {
    buildCatalogueCache();
    const rows = supplierRowsCache && supplierRowsCache.get(str(productRecord.id));
  ...
  function basketTotalCost() { return poBasket.reduce(function (sum, row) { return sum + (Number(row.cost || 0) * Number(row.qty || 0)); }, 0); }
  ...
  function showPoSuggestions(input) {
    ...
    poInput = input;
    const queryText = normalise(input.value);
    if (queryText && queryText.length < 2 && !poBasket.length) {
      poMatches = [];
      poActiveIndex = -1;
      box.innerHTML = '<div class="ci-mega-head"><div><strong>Search the supplier catalogue</strong><span>Enter at least 2 characters, or scan a full SKU or barcode</span></div></div><div class="ci-search-empty"><strong>Keep typing</strong><p>Results are indexed locally and will appear without reloading the Purchase Order.</p></div>';
      box.hidden = false;
      showSearchLayer(input);
      input.setAttribute('aria-expanded', 'true');
      positionPopover(box, input);
    ...
    box.innerHTML = '<div class="ci-mega-head"><div><strong>' + (normalise(input.value) ? 'Best supplier offers' : 'Available supplier catalogue') + '</strong><span>' + poMatches.length + ' ranked offers · current supplier and best value prioritised</span></div><div class="ci-mega-head-actions"><span class="ci-key-hint">↑↓ navigate · Enter add · Esc close</span><button type="button" class="ci-po-modal-close" data-ci-po-modal-close="true" aria-label="Close product selector">×</button></div></div><div class="ci-po-modal-search"><span class="ci-search-icon">⌕</span><input id="ciPoModalSearch" autocomplete="off" spellcheck="true" value="' + esc(input.value || '') + '" placeholder="Search product, Pool Bros SKU, supplier SKU or barcode"><button type="button" data-ci-po-modal-clear="true">Clear</button></div><div class="ci-mega-body"><section class="ci-mega-results"><div class="ci-mega-section-title"><strong>Search results</strong><span>Click Add or press Enter</span></div><div class="ci-popover-results">' + (poMatches.length ? poMatches.map(poResultHtml).join('') : '<div class="ci-search-empty"><strong>No supplier catalogue offer matched</strong><p>Try the Pool Bros SKU, supplier SKU, barcode, size or fewer product words.</p></div>') + '</div></section><aside class="ci-po-basket"><div class="ci-mega-section-title"><strong>Selected products</strong><span>' + poBasket.length + ' lines · ' + basketTotalQty() + ' units</span></div>' + basketHtml() + '</aside></div><div class="ci-po-actionbar"><div class="ci-po-action-summary"><span>Selected for this purchase order</span><strong>' + poBasket.length + ' product' + (poBasket.length === 1 ? '' : 's') + ' · ' + basketTotalQty() + ' units · ' + cash(basketTotalCost()) + '</strong></div><button type="button" class="secondary" data-ci-basket-clear="true"' + (poBasket.length ? '' : ' disabled') + '>Clear selection</button><button type="button" class="ci-po-confirm-button" data-ci-basket-confirm="' + esc(po.id) + '"' + (poBasket.length ? '' : ' disabled') + '>Add selected products to PO</button></div>';
    box.hidden = false;
    showSearchLayer(input);
    input.setAttribute('aria-expanded', 'true');
    positionPopover(box, input);
    setPoActive(0);
    const modalSearch = box.querySelector('#ciPoModalSearch');
    if (modalSearch && document.activeElement !== modalSearch) {
      const end = modalSearch.value.length;
      modalSearch.focus({ preventScroll: true });
      try { modalSearch.setSelectionRange(end, end); } catch (_) {}
    }
  }
  ...
  function closePoPopover() {
    ...
    document.body.classList.remove('ci-po-modal-open');
    tidySearchLayer();
  }
  ...
  document.addEventListener('input', function (event) {
    const modalPo = event.target.closest && event.target.closest('#ciPoModalSearch');
    if (modalPo) {
      event.stopImmediatePropagation();
      const source = document.getElementById('poProductSearch') || poInput;
      if (source) source.value = modalPo.value;
      clearTimeout(modalPo._ciTimer);
      modalPo._ciTimer = setTimeout(function () { if (source) showPoSuggestions(source); }, 140);
      return;
    }
    ...
    if (hub) {
      ...
      hub._ciTimer = setTimeout(function () { showHubSuggestions(hub); }, 160);
      return;
    }
    ...
    if (po && po.closest('.ci-po-catalogue-picker')) {
      ...
      po._ciTimer = setTimeout(function () { showPoSuggestions(po); }, 160);
    }
  }, true);
  ...
  document.addEventListener('focusin', function (event) {
    ...
    const po = event.target.closest && event.target.closest('#poProductSearch');
    if (po && po.closest('.ci-po-catalogue-picker') && (normalise(po.value) || poBasket.length)) showPoSuggestions(po);
  }, true);
  ...
  document.addEventListener('keydown', function (event) {
    ...
    if (modalPo) {
      ...
      if (event.key === 'Enter') {
        ...
        if (chosen) selectPoOffer(document.getElementById('poProductSearch') || poInput, chosen);
      }
      return;
    }
    const hub = event.target.closest && event.target.closest('#ciHubSearch');
  ...
  document.addEventListener('click', function (event) {
    if (event.target.closest && event.target.closest('[data-ci-po-modal-close]')) { closePoPopover(); return; }
    if (event.target.closest && event.target.closest('[data-ci-po-modal-clear]')) {
      const source = document.getElementById('poProductSearch') || poInput;
      if (source) { source.value = ''; showPoSuggestions(source); }
      return;
    }
  ...
  window.addEventListener && window.addEventListener('poolshed:catalogue-updated', invalidateCatalogueCache);

  window.PoolShedCataloguePerformance = {
    invalidate: invalidateCatalogueCache,
    stats: function () { const cache = buildCatalogueCache(); return { products: cache.products.length, suppliers: supplierRowsCache ? supplierRowsCache.size : 0, signature: catalogueCacheSignature }; },
    search: function (query, limit) { return rankProducts(query, limit || 10); },
    purchaseSearch: function (po, query, limit) { return purchaseOffers(po || { supplier: 'Supplier to confirm' }, query).slice(0, limit || 10); }
  };
})();
