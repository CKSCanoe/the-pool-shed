/* Pool Shed v1.11.0 catalogue and purchasing intelligence */
(function () {
  'use strict';

  const VERSION = '1.11.0';
  const RECENT_KEY = 'poolshed:v1110:catalogueRecentSearches';
  let hubPopover = null;
  let hubInput = null;
  let hubMatches = [];
  ...
  function connectedBadge() {
    ...
    return '<span class="ci-live-badge' + (online ? '' : ' offline') + '"><i></i>' + (online ? 'Live index and offline-ready' : 'Offline catalogue index') + ' · ' + activeProducts().length + ' variants</span>';
  }
  ...
  function hubCommandHtml(resultText) {
    ...
    return '<section class="ci-command-centre" aria-label="Catalogue intelligence search">' +
      '<div class="ci-command-head"><div><span class="ci-eyebrow">Pool Bros catalogue intelligence · v' + VERSION + '</span><h3>Find any product, variant or supplier code</h3><p>Typo-tolerant search across parent products, sizes, Pool Bros SKUs, supplier SKUs, barcodes, brands and descriptions. Results use the locally cached catalogue when offline.</p></div>' + connectedBadge() + '</div>' +
  ...
  function positionPopover(box, input, widthExtra) {
    if (!box || box.hidden || !input || !input.isConnected) return;
    const rect = input.getBoundingClientRect();
    const margin = 10;
    const width = Math.min(Math.max(rect.width + (widthExtra || 360), 720), window.innerWidth - margin * 2);
    ...
    const desired = Math.min(box.scrollHeight || 500, 560);
    const above = spaceBelow < 300 && spaceAbove > spaceBelow;
    box.style.top = (above ? Math.max(margin, rect.top - Math.min(desired, spaceAbove) - 8) : rect.bottom + 8) + 'px';
    box.style.maxHeight = Math.max(190, above ? spaceAbove - 8 : spaceBelow) + 'px';
  ...
  function showHubSuggestions(input) {
    ...
    box.innerHTML = '<div class="ci-popover-head"><div><strong>' + (normalise(input.value) ? 'Catalogue intelligence matches' : 'Available catalogue products') + '</strong><span>Searches products and every stock-controlled variant</span></div><span class="ci-key-hint">↑↓ Navigate · Enter search · Esc close</span></div>' + (hubMatches.length ? hubMatches.map(hubSuggestionHtml).join('') : '<div class="ci-search-empty"><strong>No catalogue result found</strong><p>Try fewer words, a size such as 5 kg, a Pool Bros SKU, supplier SKU or barcode. Common spelling errors and mixed word order are supported.</p></div>');
    box.hidden = false;
    input.setAttribute('aria-expanded', 'true');
    positionPopover(box, input, 420);
    setHubActive(0);
  }
  ...
  function showPoSuggestions(input) {
    ...
    box.innerHTML = '<div class="ci-popover-head"><div><strong>' + (normalise(input.value) ? 'Supplier offer intelligence' : 'Available supplier catalogue') + '</strong><span>Exact product relevance first, then supplier fit, cost and lead time</span></div><span class="ci-key-hint">↑↓ Navigate · Enter select · Enter again add</span></div>' + (poMatches.length ? poMatches.map(poResultHtml).join('') : '<div class="ci-search-empty"><strong>No supplier catalogue offer matched</strong><p>Try the Pool Bros SKU, supplier SKU, barcode, size or fewer product words. Products can still be assigned to a supplier from the Product Hub.</p></div>');
    box.hidden = false;
    input.setAttribute('aria-expanded', 'true');
    positionPopover(box, input, 520);
    setPoActive(0);
  }
