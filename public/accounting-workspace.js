...
(function(){
 ...
 document.addEventListener('click',e=>{if(e.target.closest('[data-open-accounting]')){closeInvoiceModal();openSidebarSubGroup('dashboard','Accounting & Xero');}});
 const baseGroups=sidebarSubGroups;sidebarSubGroups=function(id){const g=baseGroups(id).slice();if(id==='dashboard'&&!g.includes('Accounting & Xero'))g.push('Accounting & Xero');return g;};
 const baseRender=render;render=function(){if(loadedUser!==(supabaseSession?.user?.id||supabaseSession?.access_token||null)){state=null;lookups=null;}baseRender();if(active==='dashboard'&&selectedSubPage('dashboard')==='Accounting & Xero'){const screen=document.getElementById('screen-dashboard');screen.querySelectorAll(':scope > :not(.ps-section-nav)').forEach(e=>e.remove());const section=document.createElement('section');section.id='psFinance';section.className='panel';section.style.gridColumn='1/-1';screen.append(section);paint();}};
