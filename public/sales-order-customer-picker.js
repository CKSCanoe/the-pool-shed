(function () {
  ...
  function customerSignature() {
    ...
    return JSON.stringify(data.customers.map(function(c) {
      return [c.id,c.code,c.firstName,c.lastName,c.name,c.companyName,c.email,c.email2,c.email3,c.phone,c.mobile,c.postcode,c.addresses];
    }));
  }
  ...
  function createAndAttachCustomer(form) {
    ...
    toast(customerName + (salesOrder(orderId)?.customerId === newCustomer.id ? ' created and attached to ' + orderId + '.' : ' saved as a customer. The existing order customer was retained.'));
  }
